function Install-Obs {
    ...
    Ensure-Location -Path "$(Resolve-Path -Path ${ProjectDepsRoot})/obs-studio"
    ...
    $CmakeArgs = @(
        ...
        "-DCMAKE_INSTALL_PREFIX:PATH=$(Resolve-Path -Path "${ProjectRoot}/deps/obs-build-dependencies/${DepsPath}")"
        "-DCMAKE_PREFIX_PATH:PATH=$(Resolve-Path -Path "${ProjectRoot}/deps/obs-build-dependencies/${DepsPath}")"
    )
