function Install-ObsDependencies {
    ...
    if ( ! ( Test-Path function:Ensure-BuildDependencies ) ) {
        . $PSScriptRoot/Ensure-BuildDependencies.ps1
    }
    ...
    ('prebuilt', "qt${script:QtVersion}") | ForEach-Object {
        ...
        Ensure-Location -Path "$(Resolve-Path -Path "${ProjectDepsRoot}")/obs-build-dependencies"
